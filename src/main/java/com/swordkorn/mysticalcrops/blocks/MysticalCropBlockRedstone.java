 ...
 */
public class MysticalCropBlockRedstone extends BlockCrops implements IGrowable {

    public MysticalCropBlockRedstone(String unlocalisedName) {
        ...
        if((Integer) state.getValue(AGE) >= 2) {
            for(int i = 0; i < 3; i++) {
                if(state.getValue(AGE) >= 7) {
                    ret.add(new ItemStack(Items.redstone, 1));
                }
            }
        }
        for(int i = 0; i < 1; i++) {
            ret.add(new ItemStack(CropItems.redstoneSeed, 1));
        }
        return ret;
    }
}
