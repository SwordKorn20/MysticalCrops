 ...
 */
public class MysticalCropBlockIron extends BlockCrops implements IGrowable {

    public MysticalCropBlockIron(String unlocalisedName) {
        ...
        if((Integer) state.getValue(AGE) >= 2) {
            for(int i = 0; i < 3; i++) {
                if(state.getValue(AGE) >=7) {
                    ret.add(new ItemStack(CropItems.nuggetIron, 1));
                }
            }
        }
        for(int i = 0; i < 1 + fortune; i++) {
            ret.add(new ItemStack(CropItems.ironSeed, 1));
        }
        return ret;
    }
