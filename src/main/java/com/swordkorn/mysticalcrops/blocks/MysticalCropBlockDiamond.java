 ...
 */
public class MysticalCropBlockDiamond extends BlockCrops implements IGrowable {

    public MysticalCropBlockDiamond(String unlocalisedName) {
        ...
        if((Integer) state.getValue(AGE) >= 2) {
            for(int i = 0; i < 3; i++) {
                if(state.getValue(AGE) >= 7) {
                    ret.add(new ItemStack(CropItems.nuggetDiamond, 1));
                }
            }
        }
        for(int i = 0; i < 1; i++) {
            ret.add(new ItemStack(CropItems.diamondSeed, 1));
        }
        return ret;
    }
}
