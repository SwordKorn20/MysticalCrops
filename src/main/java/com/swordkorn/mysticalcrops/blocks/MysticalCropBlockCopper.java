 ...
 */
public class MysticalCropBlockCopper extends BlockCrops implements IGrowable {

    public MysticalCropBlockCopper(String unlocalisedName) {
        ...
        if((Integer) state.getValue(AGE) >= 2) {
            for(int i = 0; i < 3; i++) {
                if(state.getValue(AGE) >= 7) {
                    ret.add(new ItemStack(CropItems.nuggetCopper, 1));
                }
            }
        }
        for(int i = 0; i < 1 + fortune; i++) {
            ret.add(new ItemStack(CropItems.copperSeed, 1));
        }
        return ret;
    }
}
