 ...
 */
public class MysticalCropBlockCobalt extends BlockCrops implements IGrowable {

    public MysticalCropBlockCobalt(String unlocalisedName) {
        ...
        if((Integer) state.getValue(AGE) >= 2) {
            for(int i = 0; i < 3; i++) {
                if(state.getValue(AGE) >= 7) {
                    ret.add(new ItemStack(CropItems.nuggetCobalt, 1));
                }
            }
        }
        for(int i = 0; i < 1 + fortune; i++) {
            ret.add(new ItemStack(CropItems.cobaltSeed, 1));
        }
        return ret;
    }
}
