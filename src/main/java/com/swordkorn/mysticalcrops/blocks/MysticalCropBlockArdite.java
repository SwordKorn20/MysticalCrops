 ...
 */
public class MysticalCropBlockArdite extends BlockCrops implements IGrowable {

    public MysticalCropBlockArdite(String unlocalisedName) {
        ...
        if((Integer) state.getValue(AGE) >= 2) {
            for(int i = 0; i < 3; i++) {
                if(state.getValue(AGE) >= 7) {
                    ret.add(new ItemStack(CropItems.nuggetArdite, 1));
                }
            }
        }
        for(int i = 0; i < 1 + fortune; i++) {
            ret.add(new ItemStack(CropItems.arditeSeed, 1));
        }
        return ret;
    }
}
